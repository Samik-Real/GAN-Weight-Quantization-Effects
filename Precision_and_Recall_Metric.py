...
class DistanceBlock():
    ...
    def __init__(self, num_features, num_gpus):
        ...
        with tf.device('/cpu:0'):
            ...
            features_split2 = [] #Samik
            if self.num_gpus == 0: #Samik
                features_split2 = tf.split(self._features_batch2, 1, axis=0) #use only 1 cpu instead of splitting into gpus
            else:
                features_split2 = tf.split(self._features_batch2, self.num_gpus, axis=0)
            distances_split = []
            if self.num_gpus == 0: #Samik
                distances_split.append(batch_pairwise_distances(self._features_batch1, features_split2[0])) #There only is one element if there is no gpu
            else:
...
class ManifoldEstimator():
    ...
    def evaluate(self, eval_features, return_realism=True, return_neighbors=False):#Default: return_realism=False
...
def knn_precision_recall_features(ref_features, eval_features, nhood_sizes=[3],
    ...
    if type(precision) is tuple: #Added realism to output (Edited by Samik)
        if len(precision) == 2:
            state['precision'] = precision[0].mean(axis=0)
            state['average realism with evaluated features'] = precision[1].mean(axis=0)
            #state['minimum realism with evaluated features'] = precision[1].min(axis=0)
            #state['maximum realism with evaluated features'] = precision[1].max(axis=0)
    else:
    ...
    if type(precision) is tuple: #Added realism to output (Edited by Samik)
        if len(precision) == 2:
            state['recall'] = precision[0].mean(axis=0)
            state['average realism with reference features'] = precision[1].mean(axis=0)
            #state['minimum realism with reference features'] = precision[1].min(axis=0)
            #state['maximum realism with reference features'] = precision[1].max(axis=0)
    else:
...
def main():
    ...
    print("CNN")
    real_images_directory = 'Real_Images_Features_CNN.npy'
    generated_images_directory = 'Generated_Images_Features_CNN.npy'

    directory = 'Feature Embeddings for Precision and Recall/'

    real = np.load(directory + real_images_directory)
    generated = np.load(directory + generated_images_directory)

    with tf.Session().as_default():
        state = knn_precision_recall_features(real, generated, num_gpus=0) #With 0 GPUs (Edited by Samik)
        print(state)

    print("FC")
    real_images_directory = 'Real_Images_Features_FC.npy'
    generated_images_directory = 'Generated_Images_Features_FC.npy'
    ...
    #'''
    #Quantized
    real_images_directory = 'Real_Images_Features_CNN.npy' #real images are the same for all models!
    directory = 'Feature Embeddings for Precision and Recall/'

    print("CNN: ")
    print()
    generated_images_directory_CNN = ['Generated_Images_Features_CNN_quant3.npy','Generated_Images_Features_CNN_quant4.npy','Generated_Images_Features_CNN_quant5.npy','Generated_Images_Features_CNN_quant6.npy','Generated_Images_Features_CNN_quant7.npy','Generated_Images_Features_CNN_quant8.npy']
    for i in range(len(generated_images_directory_CNN)):
        print(generated_images_directory_CNN[i])
        print(real_images_directory)
    for i in range(len(generated_images_directory_CNN)):
        real = np.load(directory + real_images_directory)
        generated = np.load(directory + generated_images_directory_CNN[i])
        with tf.Session().as_default():
            state = knn_precision_recall_features(real, generated, num_gpus=0)  # With 0 GPUs (Edited by Samik)
            print("i value +3: " + str(i+3))
            print(state)
        print()
        print()

    print("FC: ")
    generated_images_directory_FC = ['Generated_Images_Features_FC_quant3.npy','Generated_Images_Features_FC_quant4.npy','Generated_Images_Features_FC_quant5.npy','Generated_Images_Features_FC_quant6.npy','Generated_Images_Features_FC_quant7.npy','Generated_Images_Features_FC_quant8.npy']
    for i in range(len(generated_images_directory_FC)):
        real = np.load(directory + real_images_directory)
        generated = np.load(directory + generated_images_directory_FC[i])
        with tf.Session().as_default():
            state = knn_precision_recall_features(real, generated, num_gpus=0)  # With 0 GPUs (Edited by Samik)
            print("i value +3: " + str(i + 3))
            print(state)
